<template>
    <div class="container" v-if="games">
        <!-- <Teste :item="item" v-for="item in liveGames.response" :key="item.results"></Teste> -->
        <Teste :item="item" v-for="item in games.response" :key="item.results"></Teste>
    </div>
</template>
...
<script>
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const { fetch, fetchState } = useFetch(async () => await loadGames());
            fetch();

            const getLeagues = computed(() => {
                return Object.fromEntries(Object.entries(games.value.response).sort(([a], [b]) => a.league.country - b.league.country));
            });
