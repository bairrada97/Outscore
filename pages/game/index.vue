        ...
        <div class="h2h" v-if="isH2hOpen">
            <div v-if="!isLoading">
                <div class="previousHomeTeamGames">
                    ...
                    <Game :item="item" v-for="item in previousHomeTeamGames" :key="item.fixture.id">
                        <p class="winCondition" :class="{ green: getMatchCondition(singleGame.teams.home.id, item) == 1, red: getMatchCondition(singleGame.teams.home.id, item) == 0, yellow: getMatchCondition(singleGame.teams.home.id, item) == -1 }">{{ getMatchCondition(singleGame.teams.home.id, item) == 1 ? "W" : getMatchCondition(singleGame.teams.home.id, item) == 0 ? "L" : "D" }}</p>
                    </Game>
                </div>
                <div class="previousAwayTeamGames">
                    <h2>previous {{ singleGame.teams.away.name }} Games</h2>
                    <Game :item="item" v-for="item in previousAwayTeamGames" :key="item.fixture.id">
                        <p class="winCondition" :class="{ green: getMatchCondition(singleGame.teams.away.id, item) == 1, red: getMatchCondition(singleGame.teams.away.id, item) == 0, yellow: getMatchCondition(singleGame.teams.away.id, item) == -1 }">{{ getMatchCondition(singleGame.teams.away.id, item) == 1 ? "W" : getMatchCondition(singleGame.teams.away.id, item) == 0 ? "L" : "D" }}</p>
                    </Game>
                </div>
                <div class="H2H">
                    <h2>H2H</h2>
                    <Game :item="item" v-for="item in singleGameH2H" :key="item.fixture.id"> </Game>
                </div>
            </div>
            <div v-else>
                <p>...Loading</p>
