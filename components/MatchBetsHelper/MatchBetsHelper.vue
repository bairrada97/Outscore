	...
	export default {
		...
		setup(props) {
			...
			const betsHelper = computed(() => {
				const lasthomeGames = [...getHomeTeamH2H.value].filter((item, index) => item.fixture.status.short == "FT").filter((item, index) => index < lastGamesLength);
				const lastawayGames = [...getAwayTeamH2H.value].filter((item, index) => item.fixture.status.short == "FT").filter((item, index) => index < lastGamesLength);
